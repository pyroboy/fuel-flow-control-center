...
// Import UserRole and useAuthContext
import { Truck as TruckType, TruckStatus, UserRole } from "@/types";
...
const initialMockTrucks: TruckType[] = [
  ...
  {
    id: "trk_5",
    plate_number: "FLW 555",
    capacity_liters: 8000,
    status: TruckStatus.Disabled,
    created_at: "2025-03-01T10:00:00Z",
    updated_at: "2025-04-01T17:20:00Z",
    driver: "Charlie Davis",
  }, // e.g., Major repair/decommissioned
];
// --- End Mock Data ---
...
const formatTruckStatus = (status: TruckStatus): string => {
  switch (status) {
    ...
    case TruckStatus.Unavailable:
      return "Unavailable";
    case TruckStatus.Disabled:
      return "Disabled";
    case TruckStatus.OutForDelivery:
...
const TruckRecords: React.FC = () => {
  ...
  // Check if the current user is Depot Staff
  const isDepotStaff = currentUser?.role === UserRole.DepotStaff;
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            List of registered trucks and their current status.
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                trucks.map((truck) => {
                  ...
                  return (
                    <TableRow key={truck.id}>
                      ...
                      <TableCell>
                        {truck.capacity_liters.toLocaleString()}
                      </TableCell>{" "}
                      {/* Format number */}
                        ...
                        <TableCell className="text-right">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem
                                onClick={() => handleEditTruck(truck.id)}
                              >
                              ...
                              {Object.values(TruckStatus)
                                ...
                                .map((status) => (
                                  <DropdownMenuItem
                                    key={status}
                                    onClick={() =>
                                      handleChangeStatus(truck.id, status)
                                    }
                                  >
                                    ...
                                    {status === TruckStatus.OutForDelivery && (
                                      <TruckIcon className="mr-2 h-4 w-4 text-blue-600" />
                                    )}
                                    {status === TruckStatus.Unavailable && (
